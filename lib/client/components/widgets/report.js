        ...
        var flowrateChart = (
          <div key={service.label} className="row">
            <div className='twelve columns'>
              <BoxHeader icon={'fa fa-line-chart'} title={'Tag: ' + service.label} />
              <LineChart
                data={service}
                tooltipHtml={tooltipHtml}
                xAxis={{label: 'time', tickFormat: formatTimeAxis}}
          ...
          if(_.startsWith(tagAndPid.label, service.label)) {
            ...
            _.each(perTagAndPidMemStats, function (set) {
              if (_.startsWith(set.label, tagAndPid.label)) {
                var memChart = (
                  <div className="six columns">
                    <BoxHeader icon={'fa fa-line-chart'} title={'heap usage'} />
                    <AreaChart
                      data={set.data}
    ...
    var report = (
      <div className="report">
       <div className="row foo">
         ...
         <p>Please check the server logs as we are not receiving any messages.</p>
       </div>
      </div>
    )

    if (!_.isEmpty(data)) {
      var pinsOverviewSection = this.makePinOverviewSection(data)
